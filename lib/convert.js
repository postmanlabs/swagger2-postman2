...
module.exports = {
  ...
  getMetaData: function (input, cb) {
    if (input.type === 'string') {
      return Converter.getMetaData(input.data, options, cb);
    }
    else if (input.type === 'json') {
      return Converter.getMetaData(JSON.stringify(input.data), options, cb);
    }
    else if (input.type === 'file') {
      return fs.readFile(input.data, 'utf8', function(err, data) {
        if (err) {
        ...
        return Converter.getMetaData(data, options, cb);
      });
    }

...
Converter = {
  ...
  createCollectionStructure: function(swaggerData, tree) {
    ...
    for (child in tree.children) {
      if (tree.children.hasOwnProperty(child)) {
        itemToAdd = Helpers.convertChildToItemGroup(
          swaggerData,
          tree.children[child]
        );
  ...
  convert: function(json, options, callback) {
    ...
    try {
      ...
      // options
      Helpers.options.schemaFaker = options.hasOwnProperty("schemaFaker")
        ? options.schemaFaker
        : true;
      Helpers.options.requestName = options.hasOwnProperty("requestName")
        ? options.requestName
        : "fallback";

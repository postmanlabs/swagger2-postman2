var postman_collection = require('postman-collection'),
  ItemGroup = postman_collection.ItemGroup,
  Item = postman_collection.Item,
  Request = postman_collection.Request,
  Response = postman_collection.Response,
  RequestBody = postman_collection.RequestBody,
  url = require('url'),
  yaml = require('js-yaml'),
  schemaFaker = require('json-schema-faker'),
  _ = require('lodash'),
  META_KEY = 'x-postman-meta',
  RequestAuth = postman_collection.RequestAuth,
  authMap = {
    basicAuth: 'basic',
    bearerAuth: 'bearer',
    digestAuth: 'digest',
...
module.exports = {
  ...
  convertSwaggerRequestToItem: function (swaggerData, pathItem) {
    ...
    var rUrl,
      ...
      rData = [],
      rHeaders = '',
      rPathVariables, //path param default values
      rPathVarDesc, //path param Descriptions
      rQueryDesc, //query Descriptions
      rHeaderDesc = {}, //header Descriptions
    ...
    for (param in thisParams) {
      if (thisParams.hasOwnProperty(param) && thisParams[param]) {
        ...
        if (thisParams[param].in === 'query') { // && this.options.includeQueryParams !== false) {
          ...
          rQueryDesc[thisParams[param].name] = thisParams[param].description;
          rUrl += thisParams[param].name + '=' + defaultVal + '&';
        }

        else if (thisParams[param].in === 'header') {
          rHeaders += thisParams[param].name + ': ' + defaultVal + '\n';
          rHeaderDesc[thisParams[param].name] = thisParams[param].description;
        }
          ...
          try {
            rData = this.options.schemaFaker ? schemaFaker(thisParams[param].schema) : '';
          }
          catch (e) {
          ...
          rPathVarDesc[thisParams[param].name] = thisParams[param].description;
        }
      }
    }
    ...
    //requestName
    rName = this.options.requestName === 'url' ? rUrl : operation[this.options.requestName];
    if (!rName) {
    ...
    //adding pathparam default values & description to url object
    request.url.variables.members.forEach((element) => {
      element.value = rPathVariables[element.key];
      element.description = rPathVarDesc[element.key];
    });

    //adding query param description
    request.url.query.members.forEach((element) => {
      element.description = rQueryDesc[element.key];
    });

    //adding header description
    request.headers.members.forEach((element) => {
      element.description = rHeaderDesc[element.key];
    });
    ...
    item = new Item({ name: rName, description: pathItem.request.description });
  ...
  addResponsesFromSwagger: function (swaggerData, item, resCode, swaggerResponse) {
    ...
    if (swaggerResponse.schema || swaggerResponse.headers || swaggerResponse.examples) {
      ...
      if (swaggerResponse.examples) {
        ...
        try {
          postmanResponse.body = this.options.schemaFaker ? JSON.stringify(schemaFaker(swaggerResponse.schema)
          , null, 2) : '';
        }
        catch (e) {

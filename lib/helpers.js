...
module.exports = {
    ...
    if (child.type === "group") {
      ...
      if (child.requestCount > 1) {
        ...
        for (i = 0, rCount = child.requests.length; i < rCount; i++) {
          thisItemGroup.items.add(
            this.convertChildToItemGroup(swaggerData, child.requests[i])
          );
        }
  ...
  getSingleSwaggerRequestFromFolder: function(swaggerChild) {
    ...
    for (childName in swaggerChild.children) {
      if (
        swaggerChild.children.hasOwnProperty(childName) &&
        swaggerChild.children[childName].requestCount > 0
      ) {
        return this.getSingleSwaggerRequestFromFolder(
          swaggerChild.children[childName]
        );
      }
    }
    console.log(
      "For this swaggerChild, getSingleSwaggerRequestFromFolder returns null ",
      swaggerChild
    );
  ...
  convertSwaggerRequestToItem: function(swaggerData, pathItem) {
    ...
    var rUrl,
      ...
      thisParams = this.getParamsForPathItem(swaggerData, [
        baseParams,
    ...
    _.each(operation.security, security => {
      for (var secReq in security) {
        if (security.hasOwnProperty(secReq)) {
          ...
          if (
            swaggerData.securityDefs[secReq] &&
            swaggerData.securityDefs[secReq].type === "apiKey"
          ) {
  ...
  parse: function(jsonOrString) {
    ...
    if (swaggerObj.swagger !== "2.0") {
      return {
        result: false,
        reason:
          'The Swagger object must have the "swagger" property set to 2.0',
      };
    }
    if (!swaggerObj.info) {
      return {

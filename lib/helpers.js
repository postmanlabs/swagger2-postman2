...
module.exports = {
  ...
  generateUrl: function (swagger) {
    var url = '';

    //host
    url = swagger.host ? swagger.host : '';
    // basepath
    url += swagger.basePath ? swagger.basePath : '';

    if (swagger.schemes) {
      url = swagger.schemes + '://' + url;
    }
    else {
      url = 'https://' + url;
    }
    ...
    for (param in thisParams) {
      if (thisParams.hasOwnProperty(param) && thisParams[param]) {
        ...
        if (thisParams[param].in === 'query') { // && this.options.includeQueryParams !== false) {
          ...
          rPathVariables[thisParams[param].name] = '{{' + thisParams[param].name + '}}';//defaultVal;
          this.environment.values.push({
            key: thisParams[param].name,
            value: defaultVal,
            enabled: true,
